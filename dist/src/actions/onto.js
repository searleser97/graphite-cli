...
function validateStack(silent) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            log_1.log(chalk_1.default.red(`Cannot "fix --onto", git branches must match stack. Consider running "fix" or "regen" first.`), { silent });
            process.exit(1);
        }
    });
}
function checkBranchCanBeMoved(branch, onto, silent) {
    if (utils_1.trunkBranches && branch.name in utils_1.trunkBranches) {
        log_1.log(chalk_1.default.red(`Cannot fix (${branch.name}) onto ${onto}, (${branch.name}) is listed in (${utils_1.CURRENT_REPO_CONFIG_PATH}) as a trunk branch.`), { silent });
        process.exit(1);
    }
}
function getParentForRebaseOnto(branch, silent) {
    ...
    if (!parent) {
        log_1.log(chalk_1.default.red(`Cannot "fix --onto", (${branch.name}) has no branch parent in the stack.`), { silent });
        process.exit(1);
    }
    return parent;
}
