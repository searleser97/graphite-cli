...
function regenAction(silent) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        printBranchNameStack(`(Original git infered stack)`, branch.stackByTracingGitParents(), silent);
        printBranchNameStack(`(Original graphite recorded stack)`, branch.stackByTracingMetaParents(), silent);
        // Walk the current branch down to the base and create stacks.
        yield recursiveFix(branch, silent);
        printBranchNameStack(`(New graphite stack)`, branch.stackByTracingMetaParents(), silent);
    });
}
...
function recursiveFix(branch, silent) {
    ...
    gitChildren.forEach((child) => {
        ...
        if (!oldMetaParent || oldMetaParent.name !== branch.name) {
            log_1.log(`Updating (${child.name}) branch parent from (${oldMetaParent === null || oldMetaParent === void 0 ? void 0 : oldMetaParent.name}) to (${chalk_1.default.green(branch.name)})`, { silent }),
