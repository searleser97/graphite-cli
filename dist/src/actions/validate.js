...
function validate(scope, silent) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (branch === null) {
            utils_1.logWarn("Not currently on a branch; no stack to validate.");
            return;
        }
        switch (scope) {
            case "UPSTACK":
...
function validateBranchDownInclusive(branch, silent) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (gitParents.length === 0 && metaParent) {
            throw new Error(`(${branch.name}) has meta parent (${metaParent.name}), but no parent in the git graph.`);
        }
        if (gitParents.length === 1 && !metaParent) {
            throw new Error(`(${branch.name}) has git parent (${gitParents[0].name}), but no parent in the meta graph.`);
        }
        ...
        if (gitParents[0].name !== metaParent.name) {
            throw new Error(`(${branch.name}) has git parent (${gitParents[0].name}) but meta parent (${metaParent.name})`);
        }
