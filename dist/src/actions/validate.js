...
function validate(scope, silent) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (branch === null) {
            utils_1.logErrorAndExit("Not currently on a branch; no stack to validate.");
        }
        switch (scope) {
            case "UPSTACK":
...
function validateBranchDownInclusive(branch, silent) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (gitParents.length === 0 &&
            ...
            throw new Error(`(${branch.name}) has stack parent (${metaParent.name}), but no parent in the git graph.`);
        }
        if (gitParents.length === 1 && !metaParent) {
            throw new Error(`(${branch.name}) has git parent (${gitParents[0].name}), but no parent in the stack.`);
        }
        if (gitParents.length > 1) {
            throw new Error(`(${branch.name}) has more than one git parent (${gitParents.map((b) => b.name)}).`);
        }
        if (!metaParent) {
        ...
        if (!metaParentMatchesBranchWithSameHead &&
            ...
            throw new Error(`(${branch.name}) has git parent (${gitParents[0].name}) but stack parent (${metaParent.name})`);
        }
