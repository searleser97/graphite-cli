...
class Branch {
    ...
    ref() {
        return child_process_1.execSync(`git show-ref refs/heads/${this.name} -s`)
            .toString()
            .trim();
    }
    getMetaMergeBase() {
    ...
    getTrunkBranchFromGit() {
        ...
        if (gitParents.length == 1) {
            ...
            throw new errors_1.ExitFailedError(`Cannot derive trunk from git branch (${this.name}) with two parents`);
        }
        else {
            return this;
        }
