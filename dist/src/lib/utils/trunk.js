...
function getTrunk() {
    ...
    if (configTrunkName) {
        if (!branch_1.default.exists(configTrunkName)) {
            throw new errors_1.ExitFailedError(`Configured trunk branch (${configTrunkName}) not found in the current repo. Consider updating the trunk name by running "gp repo init".`);
        }
        memoizedTrunk = new branch_1.default(configTrunkName);
        return memoizedTrunk;
    }
    ...
    throw new errors_1.ConfigError(`No configured trunk branch, and unable to infer. Consider setting the trunk name by running "gp repo init".`);
}
