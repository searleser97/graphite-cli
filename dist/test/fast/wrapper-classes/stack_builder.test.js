...
for (const scene of scenes_1.allScenes) {
    ...
    describe(`(${scene}): stack builder class`, function () {
        ...
        it("Can get just downstack from a branch", () => {
            ...
            const metaStack = new wrapper_classes_1.MetaStackBuilder().downstackFromBranch(new wrapper_classes_1.Branch("b"));
            const gitStack = new wrapper_classes_1.GitStackBuilder().downstackFromBranch(new wrapper_classes_1.Branch("b"));
            chai_1.expect(metaStack.equals(wrapper_classes_1.Stack.fromMap({ main: { a: { b: {} } } }))).to.be
                .true;
            chai_1.expect(metaStack.equals(gitStack)).to.be.true;
        });
        it("Can get branchs from a stack", () => {
            ...
            scene.repo.createChange("c");
            scene.repo.execCliCommand(`branch create "c" -m "c" -q`);
            const metaStack = new wrapper_classes_1.MetaStackBuilder().fullStackFromBranch(new wrapper_classes_1.Branch("b"));
